...
const Profile = (props) => {
  ...
  const removeDeletedAd = (id, type) => {
    if (type === "furniture")
      setItems({
        ...items,
        addedFurnitures: addedFurnitures.filter((p) => p._id !== id),
      });
    if (type === "mobile")
      setItems({
        ...items,
        addedMobiles: addedMobiles.filter((p) => p._id !== id),
      });
    if (type === "vehicle")
      setItems({
        ...items,
        addedVehicles: addedVehicles.filter((p) => p._id !== id),
      });
    if (type === "computer")
      setItems({
        ...items,
        addedComputers: addedComputers.filter((p) => p._id !== id),
      });
  };
          ...
          <div class="col">
            <div class="row">
              <div class="col-xs-10 col-md-4">
                <div>
                  <div class="card-body">
                    ...
                    <div
                      class="list-group mt-4 font-weight-normal"
                    ...
                    >
                      ...
                      {!isAdmin && (
                        <button
                          ...
                          class="btn list-group-item list-group-item-action cursor-pointer"
                      ...
                      {!itemsLoading && !isAdmin && (
                        <button
                          data-toggle="modal"
                          data-target="#edit-products-modal"
                          class="btn list-group-item list-group-item-action cursor-pointer"
                          style={{ boxShadow: "none" }}
                        >
                          <i class="fas fa-edit"></i> Edit Posted Ads
                        </button>
                      )}
                    </div>
                  </div>
              ...
              <div class="col-xs-10 col-md-8 mt-5 pt-2">
                {!isAdmin && (
                  <Fragment>
                    <h4 class="text-left">Published Ads</h4>
