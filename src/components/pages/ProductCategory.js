...
const ProductCategory = (props) => {
  ...
  let items;
  if (cat === 'furniture') items = furnitures;
  else if (cat === 'vehicle') items = vehicles;
  else if (cat === 'mobile') items = mobiles;
  else if (cat === 'computer') items = computers;
  console.log(items);
  const icon = {
    furniture: 'couch',
    mobile: 'mobile',
  ...
  return (
    <section>
      <div class='container-fluid' style={{ paddingTop: '13vh' }}>
        <div class='row p-4 bg-light justify-content-center'>
          <div class='col'>
            <div class='d-flex justify-content-center text-center m-auto align-items-center'>
              <i class={`fas fa-${icon[cat]} fa-5x`}></i>
              <div class='ml-4'>
                <h2 class='font-weight-bold text-capitalize'>{cat + 's'}</h2>
                {/* <p class="text-secondary mt-1">Showing Items: <span class="font-weight-bold">{items.length}</span></p> */}
              </div>
            </div>
          </div>
        </div>
