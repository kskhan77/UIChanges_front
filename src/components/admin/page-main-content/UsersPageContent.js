...
const UsersPageContent = () => {
  ...
  const { removeUserFurnitures, toggleUserFurnituresArchived } =
    furnitureContext;
  ...
  const setOneArchived = (userId, newUserData) => {
    setUsers(users.map((u) => (u._id === userId ? newUserData : u)));
  ...
  const deleteOneUser = async () => {
    try {
      ...
      setAlert(
        "success",
        `One user and posted ${res.data.deletedAdsNo} ads were deleted`
      );
          ...
          <div class="div border">
            <div class="border-bottom p-3">
              <p class="d-flex align-items-center m-0">
                ...
                <span class="lead text-uppercase">USERS</span>
              </p>
            </div>
            <div class="p-3">
              <Alert />
